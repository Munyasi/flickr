...
if (!is_admin()) {
     ...
     */
    add_shortcode('AFG_albums','afg_display_albums');
...
function afg_display_album_photos(){
...
$albumname="";
function album_the_title( $title ) {

    global $size_heading_map, $afg_text_color_map, $pf;

    if ('Gallery'==$title||'OCSDNET Albums'==$title ) {
         echo "<style type='text/css'>h2.page-title a, h2.post-title a, h1.page-title a, h1.post-title a {
                color: #0A828D}</style>";
        $photoset_id=$_GET['album_id'];
        $user_id = get_option('afg_user_id');
        $albumname=$pf->photosets_getInfo($photoset_id,$user_id);
        //var_dump($info);

    }
    if ( 'Gallery' == $title&&!isset($_GET['album_id'])) {
         echo "<style type='text/css'>h2.page-title a, h2.post-title a, h1.page-title a, h1.post-title a {
                color: #0A828D}</style>";
        return 'OCSDNET Albums';
    }elseif('Gallery' == $title&&isset($_GET['album_id'])){
        return '<a href="'.get_bloginfo("wpurl").'/gallery">Back to Albums</a>//'.$albumname['title']['_content'];
    }
    return $title;
}
add_filter('the_title', 'album_the_title');


/*main function that loads albums*/
function afg_display_albums(){
    ...
    if (!DEBUG) {
        //$photos = get_transient('afg_id_al_' . $id);
    }
    ...
    if ($photos === false) {
        ...
        $photos = array();
        //var_dump($pf->photosets_getInfo($photoset_id));die();
        ...
        $primary_photo_extras='url_sq, url_t, url_s, url_m,url_l,url_o,owner_name, icon_server';
        $rsp_obj_total = $pf->photosets_getList($user_id,$primary_photo_extras);

        
        $photos = array_merge($photos, $rsp_obj_total['photoset']);
        /*echo "<pre>";
        var_dump($rsp_obj_total);
        die();*/
       

    if (!DEBUG)
            set_transient('afg_id_al_' . $id, $photos, afg_get_cache_refresh_interval_secs($cache_refresh_interval));
    }else {
    ...
    $disp_gallery.="<div class='main-wrap'>";
...
/*     foreach($photos as $pid => $photo) {
          $rsp_obj = $pf->people_getInfo($user_id,' url_s, url_m, url_o');
        if ($pf->error_code) return afg_error($pf->error_msg);
        $total_photos = $rsp_obj['photos']['count']['_content'];
    }*/
    ...
    foreach($photos as $pid => $photo) {
        ...
        if(true){
             ...
             $p_title = esc_attr($photo['title']['_content']);
            ...
            if ( ($photo_count <= $per_page * $cur_page) && ($photo_count > $per_page * ($cur_page - 1)) ) {
                ...
                if ($slideshow_option != 'none')
                    $disp_gallery .= "<a $class $rel $click_event href='?album_id={$photo['id']}' title='{$photo['title']['_content']}'>";

                if ($custom_size) {
                    $timthumb_script = BASE_URL . "/afg_img_rsz.php?src=";
                    if($photo['primary_photo_extras']['width_l'] > $photo['height_l']) {
                ...
                $disp_gallery .= "<img class='afg-img' title='{$photo['title']['_content']}' style='height:159px' src='{$photo["primary_photo_extras"]["url_m"]}' alt='{$photo['title']['_content']}'/>";
        ...
        $disp_gallery.="</div>";
    //echo "<pre>";
     //svar_dump($rsp_obj_total[$flickr_api]['photo']);
   return $disp_gallery;
...
function afg_display_gallery() {
    ...
    foreach($photos as $pid => $photo) {
        ...
        if ( ($photo_count <= $per_page * $cur_page) && ($photo_count > $per_page * ($cur_page - 1)) ) {
        	...
        	foreach ($title as $key => $album) {
	    		if($album==$photo['title']){
